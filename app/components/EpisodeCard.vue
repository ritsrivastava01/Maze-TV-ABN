...
<template>
  ...
  >
    <div v-if="!episode" class="overflow-hidden rounded-2xl border border-white/10 bg-slate-800/30">
    ...
    <div v-else class="relative overflow-hidden">
      <img :src="episode.image" :alt="episode.title" class="relative z-0 block h-auto w-full" loading="lazy" />
      <div
        class="pointer-events-none absolute inset-0 z-10 bg-gradient-to-t from-black via-black/50 to-transparent"
        aria-hidden="true"
      />
      <div v-if="episode.rating" class="absolute right-3 top-3 z-20 ds-badge-rating">{{ episode.rating }}/10</div>
      <div class="absolute bottom-3 left-3 right-3 z-20 text-left">
        <p class="text-base font-black uppercase leading-none tracking-[0.04em] text-white drop-shadow-md">
          S{{ episode.season }}E{{ episode.number }}
        </p>
        <h3 class="mt-2 line-clamp-2 text-xl font-black leading-tight text-white drop-shadow-md">
          {{ episode.title }}
        </h3>
      </div>
    </div>
