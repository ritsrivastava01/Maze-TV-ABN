...
const selectedSeasonEpisodes = computed(() => {
  return seasons.value.find((season) => season.season === selectedSeason.value)?.episodes ?? [];
});
    ...
    >
        ...
        <Rail>
          <template #header>
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              <select v-model.number="selectedSeason" :aria-label="t('showDetail.selectSeason')" class="ds-select">
          ...
          <template v-if="selectedSeasonEpisodes.length > 0">
            <EpisodeCard v-for="episode in selectedSeasonEpisodes" :key="episode.id" :episode="episode" />
          </template>
          <template v-else-if="!selectedSeason && seasons.length > 0">
            <EpisodeCard v-for="n in 6" :key="`episode-skeleton-${n}`" :episode="null" />
          </template>
      ...
      <section v-if="castPreview.length > 0">
        ...
        <Rail class="!mb-0 mt-3">
          <CastCard v-for="castMember in castPreview" :key="castMember.id" :cast="castMember" />
        </Rail>
      </section>
    </div>
  </div>
