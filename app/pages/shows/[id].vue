...
const { data, error } = useFetch<ShowDetailsViewModel>(
  () => `/api/${showId.value}`,
  { key: () => `show-detail-${showId.value}`, watch: [showId] }
);
...
const panelEpisodes = computed((): Episode[] => {
  ...
  if (
    selectedSeason.value == null ||
    selectedSeason.value === data.value.seasonList[0]?.number
  ) {
...
<template>
  ...
  <div class="pb-16 pt-20 sm:pt-24 lg:pt-28">
    ...
    <section class="px-4 sm:px-6 lg:px-10">
      <div v-if="error" class="ds-panel-error">
      ...
      <template v-else>
        ...
        >
          ...
          <div class="min-w-0 md:col-span-7 lg:col-span-8">
            <ShowDetailOverview
              :show="show"
              :season-count="seasonList.length"
            />
          </div>
        </div>
      </template>
